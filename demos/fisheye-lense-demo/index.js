...
import { FisheyeLensCamera } from './FisheyeLensCamera.js';
...
class SimplexNoisePlane extends Entity {
  init(game) {
    ...
    // Create shader with 3D simplex noise and fisheye distortion
    this.shader = ShaderManager.getShader(gl, `
      ...
      void main() {
        ...
        // Apply fisheye distortion in clip space
        if (pos.w != 0.0) {
          ...
          // The distortion increases with distance from center
          float distortionFactor = 1.0 + fisheyeStrength * dist * dist;

          // Apply distortion
          ndc *= distortionFactor;

          // Restore the transformed position
          pos.xy = ndc * pos.w;
        }
  ...
  render(gl) {
    ...
    // Set fisheye strength uniform
    const fisheyeStrengthLocation = gl.getUniformLocation(this.shader.program, 'fisheyeStrength');
    gl.uniform1f(fisheyeStrengthLocation, this.game.camera.getFisheyeStrength());
...
// Create fisheye lens camera positioned to view the plane from above at an angle
const camera = new FisheyeLensCamera({
  position: [0, 2, 8],
  target: [0, 0, 0],
  fov: 45 * Math.PI / 180,
  near: 0.1,
  far: 100,
  fisheyeStrength: 0.5  // Adjust this value to control the fisheye effect
});
